import numpy as np
from tit_for_tat import TFT_improved, TFT_inertia, Nice_algo, Egoist_algo, Traitor_algo, LateNice_algo, TFT_improved_beta
from graph_flow_functions import *
...
class Agent_TFT_Beta(Agent_Graph):
    def __init__(self, ident, name, n_agents = 4, max_coop_matrix = None, debit_max = 1.0,
                 alpha_inertia = 0.6, r_incentive = 0.4, beta_adaptive = 0.6, minCost = True):
...
class Agent_TFT_Gamma(Agent_Graph):
    def __init__(self, ident, name, n_agents = 4, max_coop_matrix = None, debit_max = 1.0,
                 alpha_inertia = 0.6, r_incentive = 0.4, beta_adaptive = 0.6, gamma_proba = 0.05, minCost=True):
...
class Agent_TFT(Agent_Graph):
    def __init__(self, ident, name, n_agents = 4, max_coop_matrix = None, debit_max = 1.0,
                 alpha_inertia = 0.6, r_incentive = 0.4, minCost=True):
...
class Agent_Traitor(Agent_TFT_Gamma):
    def __init__(self, ident, name, n_agents = 4, max_coop_matrix = None, debit_max = 1.0 , t_traitor = 50):
    ...
    def act(self, coop_matrix, step_t):
        ...
        self.update_source_max(coop_matrix)
        self.compute_coop_sub_graph()
        if step_t < self.t_traitor-1:
            return self.coop_sub_graph[self.ident]
        elif step_t >= 3*self.t_traitor - 30:
            self.current_coop_matrix = self.max_coop_matrix
            self.current_source = self.source_max
            self.compute_coop_sub_graph()
            return self.coop_sub_graph[self.ident]
        else:

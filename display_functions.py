...
def show_cooperation_graph(ax, id_agents_list, mean_coop_rates, matrix_coop_rates,
    ...
    mean_coop_rates = np.array([(n * np.mean(matrix_coop_rates[i,:]) - matrix_coop_rates[i,i]) / (n-1) for i in range(n)])
...
def show_multiple_curves_sum_gain(ax, curves, optimal_gain, labels, label_other, colors, color_other, xlim, ylim, ylim_other):
...
def show_global(mean_coop_rates, 
    ...
    show_histo(axs[1, 1], current_values_gains, labels_agents, colors_agents, current_mean_gains, current_optimal_mean_gains, max_gain, max_t=max_t)
    ...
    show_multiple_curves_sum_gain(axs[1, 0], curves_gain_agents, optimal_single_gain_R, labels_agents, label_sum, colors_agents, color_sum, [0,max_t],[3,20], [0,max_sum])
